...
generated-from: rust-alc-api:3ff0f419987542dc04cef9253ac2d51b8a3c6a26
...
| crate | 役割 / 主要ルート群 |
|---|---|
| `alc-core` | 共通基盤: **共有** models / **共有** repository trait / `auth_middleware` / `tenant` / `webhook` (generic 配信) / `realtime_bus` / `redact_broadcast` / **`api_error`** (理由付きエラーレスポンス `(StatusCode, Json<Value>)` の共通ヘルパー — `bad_request`/`not_found`/`upstream_error`/`internal_error`/`internal_error_msg`。alc-auth::internal / alc-notify::lineworks_channels / lineworks_config に散っていた同型定義を Refs #668 で統合。**新しいハンドラの理由付きエラーはここを使う。同型定義を crate 内に増やさないこと**)。**`master_data` / `master_handlers` (Refs #651)**: 「テナントごとに追加できるマスタ」(id/tenant_id/name/sort_order/created_at + list は sort_order,name 順・create は sort_order 既定 0・RETURNING *) の generic CRUD。テーブル名は `MasterTable::TABLE` という**関連定数のみ**で渡す設計 (SQL 文字列連結の injection 境界を、コンパイル時に固定された値だけに絞るため)。`alc-trouble` の categories/offices/progress_statuses/task_types が最初の 4 本、`alc-maintenance` の `categories` (整備カテゴリ、既定 5 件を空テナントに auto-seed) が 5 番目の利用者。**port 化進行中 (Refs #539)**: `fcm` / `device_pair_client` は **trait (port) だけが core に残り、実装 (adapter) は alc-notify / alc-devices へ移設済み**。`auth_google` は internal OIDC 専用に縮小 (login 用 `verify()`/`extra_client_ids` は削除)。ts-rs 型 export 元。**tenko / trouble ドメインの models/trait は alc-tenko / alc-trouble へ分割済み** (Refs #513、再流入は `scripts/check_domain_split.sh` が CI で loud fail)。**新機能は原則ドメイン crate (無ければ新 crate) に置く** — alc-core に足してよいのは全ドメイン共有の基盤だけ。alc-core に struct 1 個足すと 12 test shard + 7 Builds が再ビルドされる (コストは CI 時間で可視) |
...
| `alc-misc` | health (`/health` + `/health/secret-fingerprint?name=&expected=` = 任意 env の sha256[0..8] と `expected` 突合、`{match: bool}` のみ返し oracle 防止。cross-store drift を CI で自動検出、Refs ippoan/rust-alc-api#424 / ippoan/ci-workflows#131。health_canary = JWT_SECRET drift 検知は #479 PR-3 の JWT_SECRET 全撤去に伴い削除) / measurements / employees / items / api_tokens / sso_admin / tenant_users / timecard / access_requests / staging / upload / bot_admin / members / communication_items / carrying_items / guidance_records。**一括 upsert の口は 2 つあり、上限 `MAX_BULK_UPSERT_ITEMS` (500) を `alc_core::models` で共有する** (送り手は同じ分割ロジックで投げるので、片方だけ広げるともう片方が黙って 400 を返す側になる) — 乗務員マスタの `PUT /employees/bulk-by-code` と、カード台帳の `PUT /timecard/cards/bulk-by-code` (`timecard.rs`、Refs ippoan/rust-alc-api#644。詳細は「タイムカード機能」節)。**employees の一括 upsert (Refs ippoan/alc-app-s3#125)**: `PUT /employees/bulk-by-code` (`employees.rs`) は theearth の乗務員マスタを nuxt-dtako-admin の relay が 1 日 5 回取り込む受け口で、乗務員CD (`code`) をキーに最大 500 件/リクエストをまとめて upsert する。解決順は (a) `code` 一致 (deleted_at 不問、`idx_employees_code` が deleted_at を見ない一意制約のため) → 復活 UPDATE (**ただし `nfc_id` を別の乗務員が既に持っていたら nfc_id だけ据え置いて更新し `nfc_id_conflict` で skipped に載せる** — `UNIQUE (tenant_id, nfc_id)` に当たるとトランザクションごと 500 になり 1 人のせいで全員分が落ちるため、Refs ippoan/alc-app-s3#125)。**復活 UPDATE は `driver_cd` も見る** (Refs ippoan/rust-alc-api#673) — (a) の code 検索は `SELECT id, driver_cd` で復活対象の現在値まで取り、その値を持つ**生存行が別に居る**ときだけ `driver_cd = CASE WHEN $N::BOOLEAN THEN NULL ELSE driver_cd END` で NULL に落とし、`driver_cd_conflict` を skipped に載せる (**`continue` はしない** — 復活も更新もさせる。nfc_id とも衝突していれば同じ code が `nfc_id_conflict` と 2 エントリ並ぶ = 理由ごとに 1 行が期待挙動)。後続で張る partial unique index に復活行が違反して**1 人のせいでバッチ全体が 500** になるのを防ぐガードで、SQL は分岐で複製せず bind を 1 つ増やすだけ、(b) 無ければ `nfc_id` (免許証 IC の交付日8桁+有効期限8桁=16桁) が `deleted_at IS NULL` でちょうど1行かつ code が NULL/一致のときだけ code を付与 UPDATE (2行以上/別code一致は `nfc_id_conflict` で skip)、(c) どちらも無ければ `role=['driver']` で INSERT (`ON CONFLICT DO NOTHING RETURNING id` が0行なら `unique_violation` で skip)。1 トランザクションで順次処理し `EmployeeUpsertSummary{created, updated, skipped}` を返す。repo 実装は `alc-misc/src/repo/employees.rs` の `upsert_by_code`、trait は `alc-core::repository::employees::EmployeeRepository`。**免許証まわりの更新は PUT と DELETE で意味が違う**: `PUT /employees/{id}/license` は `license_issue_date = COALESCE($1, license_issue_date)` なので **null は「変更なし」**であって消去ではなく、`PUT /employees/{id}/nfc` も必須文字列しか受けない。テスト登録した免許証を消す口が無かったため `DELETE /employees/{id}/license` (`clear_license`) を足した — 交付日・有効期限・`nfc_id` の 3 列をまとめて NULL に戻す (`nfc_id` を残すと alc-app のハブ測定値タブが 免許証 payload の nfc_id 照合で乗務員名を出し続けるため、Refs ippoan/alc-app#149)。PUT の COALESCE 意味論は 既存呼び出しが依存しているので変えていない。**測定の保存は transaction (Refs ippoan/alc-app#238)**: `repo/measurements.rs` の `create` / `update` は `TenantConn` の接続で `begin()` してから INSERT / UPDATE し、body の印 `record_as_tenko` (`CreateMeasurement` / `UpdateMeasurement` の `#[serde(default)] bool`。既存の JSON は 影響を受けず、旧サーバーは `deny_unknown_fields` が無いので無視する) が true かつ保存後の `status` が `completed` のときだけ `alc_tenko::normal_tenko::record` を呼ぶ。種別は同じ body の `tenko_type: Option<String>` (`normal` / `pre_operation` / `post_operation`、無ければ normal) をそのまま渡す — **`record_as_tenko` を文字列に変えない** (旧 web の `true` が deserialize で落ちて 400 = 測定が消える)。**この 1 か所が `alc-misc` → `alc-tenko` の 唯一のクレート依存** (循環しない。どちらも依存は `alc-core` だけ。`dependency-check` が warning を出すが job は落ちない)。**記録の作成は内側の区切り (SAVEPOINT = sqlx の入れ子 transaction) で行い、失敗したら 内側だけ rollback して `tracing::warn!` を残し、測定の保存はそのまま commit する** — 「測定は必ず残る」を 後退させないための契約で、`record_as_tenko_if_marked` がそれを担う。handler (`measurements.rs`) は `tenko_type` の値の検査 (`VALID_TENKO_TYPES`) と、電子車検証の番号 `carins_cert_no` (数字 12〜13 桁) / `carins_vehicle_id` (英数 14 桁) の形の検査 (`alc_core::repository::car_inspections::normalize_carins_numbers`、空文字は None、不正は create / update とも 400 で測定を作らない・更新しない、Refs ippoan/alc-app-s3#110) だけを持つ |
| `alc-tenko` | 点呼: tenko_call / tenko_records / tenko_schedules / tenko_sessions / tenko_webhooks / daily_health / equipment_failures / health_baselines / driver_info (alc-misc から移設)。**`driver_info.rs` (`GET /api/tenko/driver-info/{employee_id}`) は イ〜ト を 1 本の JSON で返す唯一の口** (alc-app のフロントがこれ 1 fetch で描くため、新エンドポイントを増やさず既存レスポンスに足す設計、Refs #651)。**ト 車両整備状況の `recent_maintenance_records` (Refs #c651-7)** は `alc-maintenance` に依存せず (兄弟 crate 非依存の規約、下記 `alc-maintenance` 参照) `repo/driver_info.rs` が生 SQL の 1 CTE で直接 `tenko_sessions.carins_vehicle_id`(直近セッション) → `maintenance_vehicles.car_id` → `maintenance_records` を解決する。carins 未紐づけ (NULL/空文字/該当車両なし) は空配列 (`unwrap_or_default()`、404/500 にしない)。**専用の `models` / `repository` (trait) / `TenkoState` / `overdue` (check_overdue_schedules + TenkoOverdueRepository) を自前で持つ** (alc-core から分割、Refs #513)。route は `tenant_router<S>()` generic を monolith が mount (旧 tenko-api は #556 で廃止)。**通常点呼 (`normal_tenko.rs`、Refs ippoan/alc-app#238 / ippoan/alc-app-s3#135)**: 運行者端末 (管理者ログイン無し + 端末 JWT) の「通常点呼」は点呼の段 (`submit_medical` → `submit_alcohol` → …) を踏まず measurements だけを書いていたため、運行管理者の「点呼」「点呼記録」に 1 件も出なかった。`normal_tenko::record(conn, &Measurement, &NormalTenkoInput{tenko_type, carins_cert_no, carins_vehicle_id})` が **測定の保存と同じ transaction の中で** `tenko_sessions` と `tenko_records` を 1 組作る (呼び元は `alc-misc` の測定 repository。接続を自分で取らず `conn` を受け取るのはそのため)。**種別は既定 'normal'。運行者端末で始業 / 終業を選ぶと測定の保存の body の `tenko_type` で `pre_operation` / `post_operation` が来る** (migration 140 の「1 種」を 2026-09 のオーナー判断で改めた、Refs ippoan/alc-app-s3#135。値の検査は handler で、不正値は create / update とも 400 で測定も作らない)。**点呼方法 `tenko_method='通常点呼'` は session / record 共通** (種別とは軸が違う。sessions の列は migration 141 で足した `NOT NULL DEFAULT '自動点呼'` + CHECK)。**`start_session` (`tenko_sessions.rs`) はスケジュール有無で `tenko_method` を明示的に決める (Refs #655)**: スケジュールあり (キオスク) → `'自動点呼'`、スケジュール無し (alc-app フロントのみが叩く遠隔点呼の入口) → `'遠隔点呼'` — 以前はスケジュール無し経路が `tenko_method` を渡さず DB の DEFAULT `'自動点呼'` のまま記録され、下記の血圧必須判定に巻き込まれて血圧計を使わない端末の体温提出が 400 になっていた (`create_session` の INSERT にも列を追加、bind 番号を採番し直した)。点呼執行者は空欄 (端末で完結するのでオーナー判断)。**冪等は DB に任せる** — migration 141 の部分 unique 2 本 (`uq_tenko_sessions_normal_flow_measurement` = `measurement_id` / `uq_tenko_sessions_normal_flow_employee_started` = `(employee_id, started_at)`、どちらも `WHERE tenko_type = 'normal' OR tenko_method = '通常点呼'`) + `ON CONFLICT DO NOTHING RETURNING *` で、返らなければ「記録済み」として `Ok(None)`。前者が同じ測定への完了 PUT の再送、後者が**オンラインの完了 PUT が落ちて offline 保存へ回り 測定がもう 1 件できる経路** (`started_at = measured_at` が同じなので弾ける) を受ける。**述語を `tenko_type IN (...)` に広げない** — 自動点呼・遠隔点呼の `update_alcohol` が measurement_id を UPDATE で書くので unique 違反になる。述語が 140 (`tenko_type = 'normal'`) の上位集合なのは Release Wave の切り替えの間に旧版が作る行 (normal, DEFAULT '自動点呼') も同じ index に掛けるため (旧版で記録された測定は後から pre で再送されても normal のまま 1 組)。**判定と書き込みは自動点呼と共有する (写しを作らない)**: 結果 → 状態は `tenko_sessions.rs` の `status_for_result` (`fail`/`over` → `cancelled` + 中止理由「アルコール検知」/ `pass`/`normal` → `completed` / `error`・未知・無し → `None` = **記録を作らない**)、32 列の `INSERT INTO tenko_records` は `repo/tenko_sessions.rs` の `insert_record(conn, …, tenko_method)` (`create_tenko_record` は `session.tenko_method` をそのまま渡す — 遠隔点呼へ切り替えた後の session が自動点呼として記録されないように、Refs ippoan/alc-app-s3#135。以前は `'自動点呼'` 決め打ちだった)、`record_data` + `record_hash` は `record_payload`。乗務員の名前は tx の中で引く (`get_employee_name` は自前で接続を取るので使えない)。**電子車検証の車検期限 (migration 142、Refs ippoan/alc-app-s3#110)**: 番号が片方でもあれば INSERT の前に同じ conn で `alc_core::repo::car_inspections::lookup_expiry` を呼び、tenko_sessions の 4 列 (`carins_cert_no` / `carins_vehicle_id` / `carins_expires_on` / `carins_matched_by` = `cert_no`・`car_id`・`none`、NULL = 番号なし or 照合失敗) に写す。照合は内側の SAVEPOINT で行い、**SQL が失敗しても記録は戻さない** (番号 2 列は保存、期限と matched_by は NULL、warn に番号を出さない)。**tenko_records は不変テーブルなので列を足さず**、record_data に載った 4 値を CSV が末尾 4 列 (`record_hash` の後) に出す。**自動点呼タブの血圧必須化と遠隔点呼への切り替え (migration 144、Refs ippoan/alc-app-s3#135)**: `submit_medical` (`PUT /tenko/sessions/{id}/medical`) は、`session.tenko_type == "pre_operation"` かつ `session.tenko_method == "自動点呼"` のときだけ最高・最低血圧 (`systolic` / `diastolic`) の両方が無ければ 400 で弾く。通常点呼・遠隔点呼は従来どおり空でも通す (種別は `tenko_method` の列挙で判別、状態やステータスは変えない)。**血圧が測れないときの逃げ道**が `PUT /tenko/sessions/{id}/escalate-remote` (body `{reason: String}` のみ、`EscalateToRemote`。alc-app 側 `#c135-41` が実装済みで、口の形は親 (#p135) の決定として固定) — `tenko_method` を `'遠隔点呼'` にし、切り替え時刻 (`escalated_to_remote_at` — フィールド名 = DB 列名 = JSON キーを統一。画面の管理者バッジがこのキー名の有無だけを見るため、当初 `remote_escalated_at` + `#[serde(rename)]` で JSON だけ揃えていたが、読み違いの余地を無くすため列名ごと揃え直した、Refs 親 #p135 の指摘) と理由 (`remote_escalation_reason`、画面は読まないので名前はそのまま) を記録する。status は変えない (血圧待ちの状態のまま次の医療データ提出へ進められる)。**`reason` は enum で縛らず文字列のまま受ける** (画面は選択肢から短い語を送る想定だが、選択肢が増えても API・migration を変えずに済むようにするため)。ただし空文字・空白のみと、200 文字超 (`MAX_ESCALATE_REASON_LEN`) は 400 — 上限超えは切り詰めずに拒否 (`alc-misc::employees::MAX_NAME_LEN` と同じ作法、監査に残る理由を静かに欠けさせないため)。対象は業務前 (`tenko_type == "pre_operation"`) かつ現在 `tenko_method == "自動点呼"` の session のみ — 通常点呼・**既に遠隔点呼へ切り替え済み (二重切り替え)** はどちらも対象外 (別経路の振る舞いを変えない/二重にしない)、`completed` / `cancelled` の session も対象外。どのガードも 400 (この repo の既存 handler の作法に合わせ、409 は使わない)。**141 の `tenko_method` CHECK は `'自動点呼'` / `'通常点呼'` の 2 値だったため、144 で `'遠隔点呼'` を追加**している (pg_constraint から実際の制約名を引いて DROP → 張り直す形。131 の role CHECK 拡張と同じ手順)。`TenkoSession` 構造体 (models.rs) にも `tenko_method` / `escalated_to_remote_at` / `remote_escalation_reason` を追加済み (141 の DB 列追加時は構造体側が追従しておらず、`session.tenko_method` を読む経路が無かった — 144 で埋めた)。**血圧必須は端末の `bp_enabled` が正本 (Refs ippoan/alc-app#322)**: 上記の血圧必須判定 (自動点呼 + 両方欠落 → 400) は、血圧計を繋いでいない端末 (`devices.bp_enabled = false`、migration 143) でも一律に掛かっていたため本番で詰んでいた (逃げ道の `escalate-remote` ボタンもフロント側で `bp_enabled` の端末には出ない)。`SubmitMedicalData` に `device_id: Option<Uuid>` を追加し (`bp_enabled` の真偽値自体はクライアントに送らせない — サーバが正本を引く)、`submit_medical` が `TenkoState.devices` (`Arc<dyn DeviceRepository>`、alc-tenko が新たに保持) 経由で `lookup_device_tenant` → テナント一致を確認 → `get_device_settings` の `bp_enabled` を読む。**device_id 未送信・テナント不一致・device 未発見はすべて安全側 (血圧必須のまま) に倒すフェイルクローズ** — 端末が device_id を詐称しても他テナントの `bp_enabled=false` 端末に化けられない。alc-app 側で `device_id` を body に載せる改修は別 issue (このリポジトリは受け口のみ)。**運行管理者による点呼 OK/NG 判定 (migration 148、Refs ippoan/alc-app#315)**: `POST /tenko/sessions/{id}/judgment` (`RecordManagerJudgment{judgment: "ok"|"ng", reason: Option<String>, judged_by_employee_id: Uuid}`) が `manager_judgment` / `manager_judgment_reason` / `manager_judgment_by` (employees.id への UUID FK) を書く。**status は変えない** (NG でも点呼は完了扱いのまま — `safety_judgment` の自動計算や `cancel_session` の即時中止には流用しない)。**ロール検査は 2 段構え** (この crate で唯一の前例。当初 `AuthUser.role != "admin"` 案を検討したが親レビューで却下 — `alc_api.users.role` (tenant admin JWT、admin/viewer/payroll のみ、migration 131) は `employees.role` (driver/manager/admin、TEXT[]、migration 024/028) とは別テーブル・別ドメインで、運行管理者が admin アカウントでログインしているとは限らず正当な利用者を弾く可能性があった上、法定記録として残すべき「どの運行管理者が判断したか」にもならなかった): (1) `AuthUser` (Option extractor) が入っていること = ブラウザ利用者であることだけを見る (role の値では絞らない。乗務員キオスク端末には X-User-* が無いため弾かれる)、(2) body の `judged_by_employee_id` を `driver_info` (`state.driver_info.get_employee`) で引き、同テナント・`deleted_at IS NULL`・`role` (Vec<String>) に `manager` または `admin` を含むことを検証 (無ければ 403)。CSV (`tenko_records.rs`) は tenko_records が挿入後 UPDATE 不可 (migration 015 のトリガー) なので、`list_all` が `tenko_sessions` + `employees` を LEFT JOIN して判定と判定者名 (`manager_judgment_by_name`) を都度引く (record_data のスナップショットにしない)。**キオスクの自己再開 (Refs ippoan/alc-app#351)**: 「続きから再開」(ippoan/alc-app#344) は サーバに通知せずローカルで段を復元するだけだったため、記録に数時間前の `started_at` しか残らず 実施時刻が消えていた。`POST /tenko/sessions/{id}/self-resume` (`self_resume_session`) が `resumed_at = NOW()` / `resume_reason` だけを書く。**既存の管理者用 `POST /tenko/sessions/{id}/resume` (`resume_session` + `repo.resume()`) とは別口で、そちらは無改造** — `AuthUser` 必須 + `status == "interrupted"` 必須でキオスクからは構造的に通らないが、そのガードを緩めると 管理者の中断→再開の意味が変わるため。3 点が違う: (1) **`status` を一切書かない** — `repo.resume()` は `status = $1` を無条件に書くので、再開先をリクエストから受けると tenant token だけで任意の status へ遷移できる 注入口になる。`resume_to` 相当の入力自体を作らない (キオスクは `useTenkoKiosk.ts` の `_advanceByStatus` で ローカル復元している)、(2) **`AuthUser` は `Option`** (`record_manager_judgment` と同じ作法) — 入っていれば `resumed_by_user_id` に入れ、キオスク (端末トークン) からは NULL のまま (「どの管理者が承認したか」の列なので 埋めると嘘になる)、(3) **対象 status は否定形** (`is_terminal_tenko_status` = `completed`/`cancelled`/`interrupted` でないこと)。alc-app の `RESUMABLE_TENKO_STATUSES` を Rust に写すと 登録簿が 2 つになるため。**「再開は 1 セッションにつき 1 回まで」(オーナー決定) は専用 repo メソッド `self_resume()` の `WHERE id = $ AND tenant_id = $ AND resumed_at IS NULL` で担保**し、更新 0 行 (`fetch_optional` → `Ok(None)`) なら 400 `already_resumed` — `get()` → 更新の read-then-write では 2 連打・2 端末で両方通る。**★ この条件を共有の `resume()` に足さないこと** (管理者の中断→再開が 2 回目から壊れる)。`resumed_at` は単数カラムで 2 回目の時刻を保持できないので、2 回目は新しい点呼としてやり直させる (append-only の履歴テーブルは作らない)。エラーは `alc_core::api_error` の理由付き 400 (`reason_required` / `session_not_resumable` / `already_resumed`) — alc-app が文言を出し分けるため。migration 不要 (`resumed_at` / `resume_reason` / `resumed_by_user_id` は既存列)、CSV も無改造 (`insert_record` がセッションの値をコピーする)。到達面: `tenant_router` 配下なので kiosk 専用にはならず、同テナントの他人のセッションに `resumed_at` / `resume_reason` を書けるところまで増える (status を触らない設計でそれ以上は増えない。`interrupt` / `cancel` が既に同条件で開いていることとの釣り合いで許容) |
| `alc-carins` | 車検証(carins): car_inspections / car_inspection_files / carins_files / nfc_tags。**`POST /api/car-inspections/lookup`** (body `{cert_no, car_id}`、Refs ippoan/alc-app-s3#110) は運行者端末 (kiosk) の車両の段で車検期限を照合する口で、応答は `expires_on` / `matched_by` / `car_no` (登録番号) だけ — 所有者・住所・車台番号は返さない。番号を request log の URL に載せないため GET にしない (`/{id}` より前に route)。**照合の SQL は alc-core `repo/car_inspections.rs::lookup_expiry` の 1 か所**で、通常点呼の保存 (alc-tenko) と共有する (alc-tenko → alc-carins の依存は張らない)。管理番号 OR 車両 ID で一致する行のうち期限の新しい順に 1 行 (継続検査で管理番号が変わった車も車両 ID で新しい行が引ける)、matched_by はその行で判定。期限 (YYMMDD) は月日を正規表現で検査してから `to_date` (外れたら NULL)。**2 次元コード列が空/不正な行は和暦 4 列 (元号・年・月・日) から組み立てる** (Refs ippoan/alc-app-s3#110)。元号→西暦のオフセットは SQL 直書きではなく Rust の定数表から bind する (令和 = 2018 + 年 / 平成 = 1988 + 年)、外れた値・未知の元号はエラーにせず NULL |
...
**dvr_notifications (Refs ohishi-exp/nuxt-dtako-admin#1094)**: theearth の DVR (ドライブレコーダー) 動画通知の受け皿。`dvr_notifications::internal_router` が `POST /api/dvr/notifications` (一覧のバッチ ingest) と `POST /api/dvr/files/{id}` (`.vdf` の生バイトを body ストリームで受け R2 の dtako バケットへ保存) の 2 本を持ち、`internal_shared_secret_router` 配下に merge される。**重複判定は自然キー `UNIQUE (tenant_id, serial_no, file_name)`** (migration 132、RLS + `GRANT ... TO alc_api_app`) — 動画 URL (`source_url`) は relay が 2 段のリクエストから 組み立てる派生値で theearth の形式変更で全件が新規扱いになるため**キーにしない** (列として保持するだけ)。ingest 応答の `pending` は「今回 insert された行」+「既存だが `file_status='pending'` かつ `attempts < MAX_FILE_ATTEMPTS` の行」で relay 側の retry キューを兼ねる (廃止元 rust-logi の `RetryPendingDownloads` 相当が不要になる)。`serial_no` / `file_name` は theearth 由来 = untrusted かつ R2 key (`{tenant_id}/dvr/{serial_no}/{file_name}`、既存 dtako の prefix 規約 `{tenant_id}/uploads/...` / `{tenant_id}/unko/...` に揃えた形) の構成要素なので、英数字 + `-` `_` `.` の allowlist + `..` 禁止で 400 に落とす (`valid_key_component`)。**`POST /api/dvr/files/{id}` は `{id}` を `X-Tenant-ID` と対で突合する** (`WHERE id = $1 AND tenant_id = $2`) — caller が tenant を名乗る class なので RLS 任せにせず SQL に明示する 二重防御で、「無い」と「他人のもの」はどちらも 404 にして区別させない (`tests/dvr_notifications_test.rs` が固定)。body は `Body` (ストリーム) で受けて自前で 32MB (`MAX_FILE_BYTES` = Cloud Run の HTTP body 上限) を数える — `main.rs` の `DefaultBodyLimit` はバッファリングする抽出器にしか効かないため。超過は 413 + `file_status='failed'` + `last_error`、R2 失敗は `attempts` +1。**mp4 に変換せず `.vdf` をそのまま置く**。LINE WORKS 通知はここでは出さない — 宛先の知識は dtako-scraper-relay 側にあり relay が送る (#1094 の決定 2) **乗務員の解決は `code` → `driver_cd` の順序付き 2 クエリ** (`repo/dtako_upload.rs` の `upsert_driver` / `get_employee_id_by_driver_cd`、Refs ippoan/rust-alc-api#669) — 乗務員CD は theearth 乗務員マスタ同期 (`alc-misc` の `upsert_by_code`) が `code` 列に、dtako 取り込みが `driver_cd` 列に入れるため、`driver_cd` 単独で引くと同じ乗務員が 2 行に増えていた。`OR` の 1 クエリにはしない (重複が残っている間ヒットが不定になるため)。`upsert_driver` だけが正本に `driver_cd` を バックフィルする (同じ `driver_cd` の生存行が他に居るときはしない)。既存の重複は `migrations/149_employees_dedup_by_driver_cd.sql` が `(tenant_id, driver_cd)` 単位に畳む (dtako 3 テーブルの `driver_id` を勝者へ付け替えてから敗者を soft-delete。`dtako_daily_work_hours` は `UNIQUE (tenant_id, driver_id, work_date, start_time)` を持つので、勝者が同じ時刻の行を既に持つ場合は動かさず敗者に残す = 次回取り込みで作り直される)。**違反しうる書き手を先に塞いでから index を張る**順序で入れた (Refs ippoan/rust-alc-api#673)。ガード側: 復活時の `driver_cd` 衝突は NULL に落として `driver_cd_conflict` で skipped に載せ (`alc-misc` の `upsert_by_code`)、`upsert_driver` の新規 INSERT は **target 無しの `ON CONFLICT DO NOTHING`** + `fetch_optional` にして 0 行なら生存行を引き直す (引けなければ乗務員解決は `None`)。index 本体は `migrations/150_employees_unique_driver_cd.sql` = **`CREATE UNIQUE INDEX idx_employees_tenant_driver_cd_live ON alc_api.employees (tenant_id, driver_cd) WHERE driver_cd IS NOT NULL AND deleted_at IS NULL`** で、前段に**生存重複の事前確認**を持つ (1 組でもあれば `RAISE EXCEPTION`。**畳み直さない** — 149 が統合済みなので 0 件のはずで、0 でなければ「149 の後に入った新しい重複」という別の事実になり、dedup ロジックを 2 本持つと正本が分からなくなる。`DETAIL` に重複した組を先頭 20 組まで載せるので CI / staging のログだけで原因が読める)。述語の `deleted_at IS NULL` は soft-delete 済みの行が `driver_cd` を握り続けるのを避けるため、`driver_cd IS NOT NULL` は theearth 同期だけで作られた未設定行を index から外すため (先例: `006_employee_code.sql` の `idx_employees_code`)。**この index が在ると 149 の DO ブロックはもう流せない** — 敗者を soft-delete する前に勝者へ `driver_cd` をバックフィルする UPDATE が生存中の敗者と衝突するため、`tests/dtako_driver_dedup_test.rs` から **149 を実 DB で流し直していたテスト 3 本は削除した** (跡地にコメントを残してある) — 149 は適用済みで checksum に凍結されており中身が変わることは機械的に不可能、まっさらな DB では空の `employees` に対して無害に走るだけなので、テストの中から本番の制約を DROP してまで守る回帰が無い。再発防止として効いているのは index そのもの (F 節) と復活時のガード (E 節)。 **運行の変更記録 (`dtako_operation_changes.rs` + `repo/dtako_operation_changes.rs`、migration 152、Refs ohishi-exp/nuxt-dtako-admin#1133)**: 運行を消す口は 2 つ — 上げ直し (`process_zip` → `replace_operation`) と手動削除 (`DELETE /api/operations/{unko_no}` → `delete_by_unko_no`、crew_role をまとめて消す)。どちらも**同じトランザクションで**旧行を読んでから消し、前の値を `dtako_operation_changes` に残す (上げ直しは新行と比べて違うときだけ `reason='reupload'`、初回取り込みは記録しない。手動削除は crew_role ごとに `reason='manual_delete'`・`after=NULL`)。`process_zip` の `delete_operation` + `insert_operation` はこの `replace_operation` 1 本に置き換わった。比べる値は `driver_cd` (`COALESCE(employees.driver_cd, code)`) / `departure_at` / `return_at` (UTC 文字列) と、KUDGIVT の区間時間をイベントCD の既定分類 (`default_classification`) で合計した `drive/cargo/break/rest_minutes`。**`dtako_operations` の `op_*` 列は 054 で定義されただけで一度も書かれておらず全行 NULL** なので使わない。上げ直しの before の分数は **split で上書きされる前の R2 旧 KUDGIVT** (`fetch_and_parse_kudgivt` 流用。split は `process_zip` の後に走るのでこの時点では前回の値)、after は今回の zip の KUDGIVT。旧 KUDGIVT が取れない回は before に分数の代わりに印 `before_kudgivt: "unavailable"` を残し (黙って「変化なし」に見せない)、比較は before に在るキーだけ・印は除外 (`snapshot_changed`)。R2 は旧行が在るとき (`operation_exists`) だけ読む。表は追記専用で `alc_api_app` には SELECT, INSERT のみ。読み口 `GET /api/dtako/operation-changes?driver_cd&from&to` は記録の `driver_cd` 列か before/after の `driver_cd` で引き、期間は recorded_at ではなく**運行の日付** (unko_no 先頭 6 桁、読めなければ departure_at) で Rust 側で絞る。`recording_since` = 表の最古 recorded_at。実 DB テストは `tests/dtako_operation_changes_test.rs`。 |
...
| `alc-maintenance` | 車両整備記録 (maintenance、国交省 遠隔点呼要件 2-四-ト対応、Refs #651)。`vehicles` (車両マスタ CRUD + carins 紐づけ) + `categories` (整備カテゴリ) + `records` (整備記録、Refs #c651-5) + `files` (添付ファイル、Refs #c651-6) を持つ。表は migration 147 (maintenance_vehicles / maintenance_categories / maintenance_records / maintenance_files) で一括用意済み。**車両 identity の正本は `maintenance_vehicles`。電子車検証 (carins) は従属する任意のリンク** — `car_inspection` の UNIQUE は車検証 1 枚ごとの履歴表 (`(tenant_id, "ElectCertMgNo", "GrantdateE","GrantdateY","GrantdateM","GrantdateD")`) で `(tenant_id, "CarId")` は index のみのため FK の参照先にできず、carins の行があるのは一部テナントだけなので必須にしない。`car_id` は `UNIQUE (tenant_id, car_id) WHERE car_id IS NOT NULL` の partial unique (2 台が同じ車検証を主張するのを防ぐ、違反は 409)。`registration_number` (自動車登録番号) は移転で再割当てされるため UNIQUE にしない。**`PUT /api/maintenance/vehicles/{id}/carins`** は既存の `alc-core::repository::car_inspections::CarInspectionRepository::lookup_expiry` (実体は `alc-carins` の `PgCarInspectionRepository`) を in-process で呼ぶだけで、carins へ HTTP は張らない。`matched_by` が `"none"` なら 400。`cert_no` だけの入力 (car_id 未指定) のときは、一致した行の実際の `CarId` を別クエリ (`fetch_car_id_by_cert_no`) で引いて保存する (`lookup_expiry` 自体は期限照合のみで `CarId` を返さないため)。**`GET .../carins-candidates`** は `registration_number` を全角→半角に正規化 (`alc-trouble` の `trouble_tickets.rs` の一覧検索 `translate(...)` と同じ文字集合、ただしこちらは server 側で 1 回だけ) してから `car_inspection` の `EntryNoCarNo`/`CarNo` を **「正規化済み登録番号がその値を含むか」** (逆方向にすると空文字列の候補が `%` 相当になり全件ヒットする点に注意) で照合し、同一 `CarId` は最新の交付日の 1 行に畳む (`DISTINCT ON`)。 **`GET .../carins-import-candidates` / `POST .../carins-import` (Refs #662)** は**まだ `maintenance_vehicles` に取り込まれていない carins** を一覧し、選ばれたぶんを **1 トランザクション**で取り込む口 (車両一覧からの一括取り込み UI が使う)。候補一覧は `carins-candidates` の SQL の**反転**で、履歴表を最新 1 枚に畳む `DISTINCT ON (ci."CarId") … ORDER BY "GrantdateY" DESC, …` ・`COALESCE(NULLIF("EntryNoCarNo",''), "CarNo")` ・全角の `translate` をそのまま流用する。**突き合わせは Rust 側の `normalize_registration_number` ではなく SQL の `translate`** (単体の候補検索と違い N 件を SQL 内で畳むため)。返す `CarinsImportCandidate` は `car_id` / `cert_no` / `car_no` / `existing_vehicle_id` (`Option<Uuid>`、NULL = 未登録) の 4 つだけ — 所有者・住所・車台番号は返さない (`CarinsCandidate` と同じ最小方針)。`car_id` で既に紐づけ済みのものは除外する (partial unique が `deleted_at` を見ないので、soft delete 済みの行が持つ `car_id` も除外対象)。取り込みは「登録番号一致の既存車両があれば `car_id` + `carins_linked_at` を set / 無ければ作成 / 既に紐づけ済み・一致先が別の車検証に紐づけ済みなら skip」で、`{created, linked, skipped}` の**件数のみ**返す (行は返さない)。**フロントに「POST で作成 → PUT .../carins で紐づけ」の 2 コールをさせない** — 間で失敗すると裸の車両行が残り、`registration_number` の index は UNIQUE ではない (147:34。UNIQUE は 147:30 の `(tenant_id, car_id) WHERE car_id IS NOT NULL` だけ) ため再試行のたびに重複行が増える。UNIQUE 違反 (23505) は SAVEPOINT (sqlx の入れ子 transaction、`alc-misc::repo::measurements` が手本) で囲んで skip 扱いにし、同時実行でも冪等にする。**新 SQL は `car_inspection` 側と JOIN する `maintenance_vehicles` 側の両方の WHERE に `tenant_id` を明示する** (RLS 任せにせず、片肺にしない)。route は `/{id}` (`Path<Uuid>`) より**前**に並べる (`alc-carins` の `car_inspections.rs:30-32` と同じ作法)。専用の `models` / `VehiclesRepository` (trait) + `PgVehiclesRepository` (実装) / `MaintenanceState` を自前で持つ (`alc-trouble` の `repository`/`repo` 分割を 1 ファイル `vehicles.rs` にまとめた形。mock test 差し替え用に repo は trait object)。route は `Router<MaintenanceState>` を monolith が `.with_state` でマウント。**整備カテゴリ (`categories.rs`)** は `alc-core::master_data` の generic master にテーブル名 (`maintenance_categories`) だけ差し替えて乗る 5 番目の利用者 — 空のテナントで`GET /api/maintenance/categories` を叩くと既定 5 件 (定期点検/修理/部品交換/タイヤ交換/オイル交換) を auto-seed する (`alc-trouble/src/categories.rs` と同じ作法)。**`records.rs` (整備記録、Refs #c651-5)**: `GET/POST /api/maintenance/records` + `GET/PUT/DELETE /api/maintenance/records/{id}`。一覧フィルタは `vehicle_id` / `category_id` / `date_from` / `date_to` (`performed_on` に対して) / `q` (`description`/`vendor` 部分一致)、既定の並びは `performed_on DESC, created_at DESC` (`alc-trouble::repo::trouble_tickets` の動的 WHERE 句組み立てと同じ作法)。`cost` (NUMERIC) は `::text` キャストして文字列で受け取る (`TroubleTicket.damage_amount` と同じ)。**★ `vehicle_id` / `category_id` は DB の FK が「行が存在すること」しか保証せずテナントの一致は保証しない** — 作成・更新時に `vehicle_belongs_to_tenant` / `category_belongs_to_tenant` (確認クエリにも `tenant_id` 述語を明示、RLS 任せにしない) で確認し、無ければ 500 ではなく 400 を返す。`category_id` の検証は `categories.rs` の generic master 実装には依存せず `maintenance_categories` を直接クエリする。`RecordsRepository` trait + `PgRecordsRepository` を `vehicles.rs` と同じ 1 ファイル構成で持つ。**`files.rs` (添付ファイル、#c651-6)**: `POST/GET /api/maintenance/records/{record_id}/files` (multipart 添付 / 一覧) / `GET /api/maintenance/files/{file_id}/download` / `DELETE /api/maintenance/files/{file_id}` (ソフト削除)。新しい `*_R2_BUCKET` は足さず、`src/main.rs` の既定の共有 `storage: Arc<dyn StorageBackend>` に prefix `{tenant_id}/maintenance/{record_id}/{uuid}.{ext}` で相乗りする (`MaintenanceState.storage` は `alc-trouble` の `trouble_storage` と同じ `Option` 設計。未設定は 503 fail-closed)。署名 URL は使わずサーバが bytes を返す。サムネイル生成はしない。**`record_id` / `file_id` はどちらも `tenant_id` を明示述語で確認してから storage を引く** (`storage_key` のテナント ID は認可の根拠にしない、RLS 任せにしない) — `crates/alc-trouble/src/files.rs` が手本 |
| `alc-notify` | LINE/LINE WORKS 配信: recipients / groups / documents / distribute / ingest / line_config / line_webhook / lineworks_* / read_tracker / viewer / email_documents / extract / redact / background_extract / background_redaction。**`line_webhook` は #434 lockdown で internal_router 併設**: `/api/internal/notify/line/webhook` (`require_internal_jwt`、auth-worker の public 受け口が OIDC mint で forward)。署名検証 (全テナント channel secret 照合) は rust 側で、**`list_enabled_line_configs()` SECURITY DEFINER 関数経由で RLS バイパス** (migration 117。生クエリだと未認証パスで `app.current_tenant_id=''` → `''::UUID` キャストが 500 する既知罠)。**ただし 072 の `FORCE ROW LEVEL SECURITY` があると所有者=SECURITY DEFINER 実行ロールにも RLS が効いて関数経由でも 500 するため、migration 118 で `NO FORCE` にして所有者バイパスを効かせている** (devices は元から FORCE 無し)。app ロール (非所有者) には RLS 維持。`public_router` (`/notify/line/webhook`) は LINE Console URL 切替 + allUsers 削除までの移行期間 dual-mount。**`lineworks_channels` も internal_router を持つ** (`/api/internal/lineworks/bot-secret/{bot_id}` / `/api/internal/lineworks/event` / `/api/internal/lineworks/send`、いずれも `require_internal_jwt` 配下)。`POST /api/internal/lineworks/send` (body `{channel_id?: Uuid, recipient_id?: Uuid, text: String}`、Refs ohishi-exp/nuxt-dtako-admin#874) は **無人 worker (dtako-scraper-relay の netprint cron) 用のテキスト送信口**。同等の tenant 経路 `POST /notify/lineworks/channels/{id}/test-send` は `require_tenant_header` 配下で、auth-worker の `alc-internal-proxy` が tenant 経路の forward を禁じている (shared secret だけで `X-Tenant-ID` を詐称でき #434 の再現になる) ため、cron からは叩けない — 迂回せず internal 側に口を足した形。**宛先は `channel_id` (トークルーム) と `recipient_id` (個人) の 2 択で、どちらか一方が必須**。両方指定は 400 `target_ambiguous` / 両方省略は 400 `target_required` で弾く — 片方を優先する実装にすると呼び出し側の設定ミスが「意図しない相手に届いた」として現れるため。**キー無しと明示 `null` はどちらも「未指定」**扱い (`Option<Uuid>` の serde 既定。caller がどちらの書き方でも通るように、mock test で HTTP 越しに縛ってある)。**recipient 宛は後から足した (#874 の 9)**: 実運用ではトークルームが 1 件も登録されておらず (Bot が招待されていない)、`notify_recipients` の個人が宛先だった。**新パスではなく body の拡張にしたのは、パスを増やすと auth-worker の `alc-internal-proxy` allowlist にも PR が要るから** (body 拡張なら auth-worker は無変更)。**`channel_id` は `lineworks_channels` の行 id (Uuid) であって LINE WORKS 側の channel 文字列ではない**。`recipient_id` は `notify_recipients` の行 id。internal 経路は `X-Tenant-ID` を honor しないので tenant はどちらも行から解決する: `lineworks_channels` は FORCE RLS (migration 102) で素引きが 0 行になり、`notify_recipients` も実行ロール `alc_api_app` が非所有者なので RLS (069) で 0 行になるため、SECURITY DEFINER 関数 `lookup_lineworks_channel_for_send` (migration 129) / `lookup_notify_recipient_for_send` (migration 130) 経由の `get_for_send` でバイパス取得する (`trouble_schedules::get_for_fire` / `notify_deliveries::get_for_view` と同じ作法)。`active = FALSE` (Bot 退出済み) の channel 行も返す — 404 に潰すと 「id が無い」と「Bot が居ない」を呼び出し側が区別できなくなるので、上流に到達させて 502 にする。recipient 側のエラー契約: 行が無ければ 404 `recipient_not_found` / `provider != "lineworks"` (= `lineworks_user_id` が NULL) は 400 `recipient_not_lineworks` (LINE 宛は別の Messaging API 経路なのでここでは扱わない。**黙って何もしないと呼び出し側が「送れた」と誤認して通知が消える**) / `enabled = false` は **404 ではなく 400 `recipient_disabled`** (行は在るので id 違いと混同させない。無効化は「もう送るな」の意思表示で、tenant 経路の `distribute` が `list_enabled` で無効宛先に配らないのと揃える)。SECURITY DEFINER 関数側で `enabled` を絞らないのはこの区別を残すため。**recipient には `lineworks_channels.bot_config_id` に相当する列が無い** (宛先は個人であって Bot に紐づかない) ので、送信 Bot は `distribute::resolve_lineworks_config` と同じく tenant の `provider = "lineworks" AND enabled` な bot_config から選ぶ (`pick_lineworks_bot_config_id`)。**0 件なら 500 `bot_config_not_found` / 複数件なら `list_configs` の順 (`ORDER BY name`) の先頭** — internal 経路と `distribute` が別の Bot から送るとログの追跡が壊れるので、選び方を揃えること自体が契約。Bot ごとに宛先を分けたくなったら `notify_recipients` に bot_config_id を足す (ここで別ルールを足さない)。bot config の取得 + 復号は private fn `resolve_bot_config` に括り出して channel 宛 (`send_text_via_channel`、tenant 経路の test-send と共用) と recipient 宛 (`send_text_to_lineworks_user` → `send_text_to_user`) が共有する。**`lineworks_config.rs`** (Refs #540) — bot config 解決の共通ヘルパー `resolve_lineworks_config` (`lineworks_directory.rs` と `lineworks_login_activity.rs` が共有。`distribute.rs::resolve_lineworks_config` は戻り値の型が違う別実装で未統合)。**`lineworks_login_activity.rs`** (`GET /notify/lineworks/login-activity?days=N`、Refs #540) — LINE WORKS 監査ログ (`audit.read` scope、`LineworksBotClient::fetch_login_audit_csv`) からログイン履歴を取得し、`list_org_users` (`directory.read`) の組織メンバーと突き合わせて N 日以上ログインの無いメンバーを確認する (`/admin/notify` の「ログイン状況」タブ用、確認のみで自動送信は無い)。**CSV の列構成は公式ドキュメント未記載だが 2026-09-11 に実データで確認済み**: `説明,メンバー,日時,IPアドレス,ログイン方法,サービスタイプ` の6列で、「メンバー」は `"表示名 (email@example.com)"` 形式・「説明」に成否が文章で入る (「結果」列は無い)。`parse_login_audit_csv` はキーワードでヘッダ列を探し、未知の見出しは `csv_columns_not_recognized` で失敗する設計。「並行して呼び出さないでください」制約への対応はプロセスローカルな mutex + 5分キャッシュ (複数インスタンスでは効かない)。**取得期間 (`AUDIT_WINDOW_DAYS`) は API 上限の 31 ではなく 30**: ちょうど31*24hを指定すると LINE WORKS 側が `400 LIMIT_EXCEEDED "Period must be within 31 days."` を返す (2026-09-11 本番実測。開始・終了の暦日をまたぐと「31日超」と判定されるらしく境界値ぴったりは弾かれる)。**★ 監査ログダウンロード URL (302 → `Location`) は Location 先にも同じ `Authorization: Bearer` を付け直す必要がある** (2026-09-11 本番実測: `audit.read` scope は正しく有効化されていたのに `401 "Authentication failed."` になっていた。`reqwest::Client` の既定リダイレクト追従はクロスホストで `Authorization` を落とすため。`LineworksBotClient` は `no_redirect_client` (リダイレクト非追従) + 手動 `Location` 追従 (`get_with_auth_redirect`) で対応。LINE WORKS Developers コミュニティに同一症状の報告・解決事例あり: forum.worksmobile.com/jp/posts/101257)。**★★ `service=auth` 単体では実態を反映できない** (本番実測: 187人中167人が「記録なし」— モバイルアプリはセッション保持でログインイベントを出さないため)。`service=message` (トーク送信、LINE WORKS 監査ログは送信の記録しか無く受信/既読は取れないとコミュニティ報告あり) + 掲示板既読 API (`/v1.0/boards` → `/{id}/posts` → `/{id}/posts/{id}/readers`、`board.read` scope、既読日時は取れず true/false のみなので「投稿日時以降にアクティブだった」という**下限値**として扱う) を合成して精度を補う設計にした (`fetch_audit_csv(service)` に一般化 + `fetch_board_activity_lower_bound`、Refs #540)。**各行は合成値 `last_login_at` と一緒に、その値を出した信号 `LoginActivityEntry.last_login_source` (`"auth"`/`"message"`/`"board"`、記録なしは `null`) を返す** (画面の「根拠」列と絞り込み、#540 フォローアップ。値は auth-worker `/admin/notify` の `la-source` の option value と一致させる)。信号単体の日時は返さない (source と一致する信号の値は `last_login_at` と同じで重複するため)。合成は純粋関数 `combine_activity` で、戻り値 `CombinedActivity` は `combined` (last_login_at/stale 判定の実体) に加えて `auth`/`message`/`board` の信号別内訳を保持し、純粋関数 `activity_source` が `combined` と**等しい**信号を auth → message → board の順で探して `last_login_source` を決める (同時刻なら auth > message > board)。**board API 3種のレスポンス shape は 2026-09-11 に本番で実測済み**: 投稿一覧は新しい順・`createdTime` は RFC3339 (`+09:00`)・既読者 API は query 無しでメンバー全員が `isRead` 付きで 1 ページに返る。**投稿は月〜四半期に 1 回の頻度のため、30 日窓 (`AUDIT_WINDOW_DAYS`) では掲示板既読の下限値は通常空 (`last_login_source` が `"board"` になる行は通常無い)** — best-effort であり不具合ではない (同日の実測で全掲示板の窓内投稿が 0 件。窓・`BOARD_ACTIVITY_MAX_*` はユーザー判断で据え置き)。各段の件数 (掲示板数・窓内投稿数・RFC3339 として読めず捨てた数・既読者数・突合で残った数) と、boards/posts/readers のキーが無いときの warn はログに出るので、0 件になった段は Cloud Run ログで切り分ける — auth/message は必須(失敗でエンドポイント全体がエラー)、message/board は best-effort (失敗はログ警告のみで無視、呼び出し回数は `BOARD_ACTIVITY_MAX_BOARDS`/`_MAX_POSTS_PER_BOARD` で上限)。それでも「読むだけで何も送信・閲覧しないメンバー」は原理的に判定できない (LINE WORKS の監査ログ設計上の限界)。**`service=message` の実ヘッダーは `日時,送信者,受信者,チャンネルID,イベント` (2026-09-11 実測) — `service=auth` と違い「メンバー」列ではなく「送信者」列**なので `parse_login_audit_csv` の識別列キーワードに `送信者`/`sender` も含める (受信者は拾わない — 能動的に送った証拠だけを使う)。**board API 3種は最初の実装でレスポンス構造が外れて `error decoding response body` で全滅した** — 原因の特定を早めるため `list_boards`/`list_recent_board_posts`/`list_board_post_readers` は `.json()` 直呼びをやめ、共通の `get_json` (`get_with_auth_redirect` の上で `serde_json::from_slice`) に寄せ、失敗時に実body (`truncate_for_log` で500字に切り詰め) をエラーメッセージへ埋め込む設計にした。次にズレていたらこのログで一発診断できる。**実際に取れた本番エラーで判明した原因: `boardId` は文字列ではなく JSON の数値** (`4000000000000000001` のような64bit整数、実装時は文字列と誤って推測していた)。`deserialize_id_as_string` (文字列・数値どちらも受け付けて `String` に正規化する visitor、`deserialize_expires_in` と同じパターン) を `boardId`/`postId`/(念のため) `userId` に適用して解消。**LINE WORKS API は ID 系フィールドの型 (文字列 vs 数値) が namespace によって不統一** — Directory Users API の `userId` は UUID 文字列、掲示板の `boardId`/`postId` は数値、という前例ができたので、新しい ID フィールドを追加するときはこの `deserialize_id_as_string` を先に使っておくと安全 |
| `alc-devices` | デバイス登録 (`devices`)。kiosk 端末 re-pair (再認証、Refs #495) は `re_pair_policy.rs` (pure 判定 fn) + `devices.rs` の `authorize_repair`/`re_pair` handler。auth-worker `/device/pair-internal` 呼び出しは port/adapter 分離 (Refs #539): trait `DevicePairClient` + `PairedCredential` は `alc-core::device_pair_client` (AppState が trait object を保持するため)、実装 `HttpDevicePairClient` は `alc-devices::device_pair_client`。合成は main.rs。設計 SoT: `docs/plan-device-repair.md`。**pairing-tenant internal route (Refs ippoan/auth-worker#544)**: `GET /api/internal/devices/{device_id}/pairing-tenant` (`devices::internal_router`、`require_internal_jwt` 配下 = aud=alc-api-internal) が device_id から登録済みで有効な端末の tenant_id を返す。auth-worker の pair-internal がこれで端末の tenant を決める。`DevicePairClient::mint` は `(device_id, label)` を取り、pair-internal の本文に `tenant_id` を載せない (auth-worker が device_id から tenant を決めるため。実装は `alc-devices::device_pair_client`、呼び出し元の re-pair handler が渡す)。**hub_measurements (Refs #564)**: CoreS3 (alc-app-s3) ハブ測定の ingest。`hub_measurements::internal_router` (`POST /api/hub/measurements`、`internal_shared_secret_router` 配下 = X-Internal-Shared-Secret + X-Tenant-ID) がバッチ/単発を受け、kind allowlist (`HUB_MEASUREMENT_KINDS`: temperature/blood_pressure/alcohol/fc1200_raw/license/timecard (license は CoreS3 が点呼開始時に免許証 IC を読み取って同じ session_id で送るもの、Refs ippoan/alc-app-s3#125。timecard は NFC タイムカード端末の打刻イベントで payload は `{card_id, card_kind}`、card_id は接頭辞なしの生値・session_id は載らない、Refs ippoan/alc-app-s3#134)、DB CHECK は意図的に無し = 将来 kind はコード変更のみ。**allowlist に無い kind は 400 → recorder が error 応答 → 端末が ack されず無限再送 + 送信キュー溢れ**になるので、端末より先にここへ足すこと) を検証して `PgHubMeasurementsRepository` (repo/hub_measurements.rs) が `ON CONFLICT (tenant_id, device_id, seq) DO NOTHING` で冪等 insert (migration 126、RLS tenant 分離)。`insert_batch` の戻り値は `HubMeasurementsIngestResponse` (`inserted` / `duplicates` の件数)。**社員解決の凍結 (Refs ippoan/alc-app-s3#134)**: `kind="timecard"` は **insert の前に** `freeze_employee_id` がカードから社員を解決し、結果を `payload.employee_id` に入れてから INSERT する。**読み出しはこれを読むだけで JOIN も再解決もしない** — 再解決すると、`timecard_cards` が hard DELETE + `UNIQUE (tenant_id, card_id)` で「削除 → 再登録」でしか付け替えられないため、**退職者のカードを新人に回した瞬間に退職者の過去の打刻が全部新人に付く**。凍結の実体は `ON CONFLICT (tenant_id, device_id, seq) DO NOTHING` で、再送しても最初に入った payload が残る。カード照合は `alc-core::repository::timecard::resolve_employee_by_card` (timecard_cards → employees.nfc_id フォールバック。**カードが見つかっても `employee_id` が NULL (保留行) ならフォールバックへ落ちる**、Refs ippoan/rust-alc-api#644) で **`alc-misc::timecard::punch` (ブラウザ版) と共有**する 1 か所。**★ 端末が payload に載せた `employee_id` は kind を問わず必ず捨てる** (`strip_client_employee_id`) — 残すと端末が任意の社員に打刻を付けられ、「端末の申告する tenant を信じる経路を新設しない」という不変条件が payload 経由で破れる。**解決できなくても測定行は入れる** (未登録カードも DB 障害も warn/error のログだけで ingest は 201)。行が残っていれば `employee_id` の無いものだけを埋め直す backfill を後から書けるので、**ここに独自の再送・キュー・リトライ機構を足さないこと** (backfill は既存値を絶対に上書きしないこと)。trait は AppState 配線のため `alc-core::repository::hub_measurements`。ingest 前に `ensure_tenant_for_staging` (Refs #567): staging の揮発 DB で device JWT 由来 tenant が dangling になり FK 違反 500 になるのを、STAGING_MODE 限定で tenant 冪等作成 (`auth.ensure_tenant_exists`) して救済 — seed (staging/entrypoint.sh) への tenant ハードコードは撤回、本番は no-op。経路: CoreS3 →(WSS+device JWT)→ cf-alc-recorder (ippoan/alc-app) →(service binding)→ auth-worker /alc-internal-proxy → 本 endpoint。**読み出し (Refs #592)**: `hub_measurements::tenant_router` (`GET /api/hub/measurements`、テナント認証付き router = X-Tenant-ID) が同じテーブルを `created_at DESC` で返す。ingest 用の `internal_router` とはパスが同じでも**別 router** — あちらは cf-alc-recorder 専用の shared-secret 経路なので混ぜない。絞り込みは `device_id` / `kind` (allowlist 外は 400、typo を無言の 0 件と区別するため) / 期間 `from`・`to` (**`created_at` に対する閉区間**、`recorded_at` は端末の時計未同期で NULL になり得るため基準に使わない。`from > to` は 400)、ページングは `limit` (既定 50 / 上限 200 に clamp、実効値をレスポンスに返す) + `offset`。**総件数 (COUNT(*)) は返さない** — ingest テーブルは伸び続けるため。代わりに repo が `limit + 1` 件を引き handler が `has_more` に畳む。SQL は RLS 任せにせず `tenant_id = $1` を明示 (index `hub_measurements_tenant_device (tenant_id, device_id, created_at DESC)` の先頭列に乗せる二重防御)。テナント分離は `tests/hub_measurements_test.rs` / `tests/mock_tests/mock_hub_measurements_test.rs` の両方で固定済み。`payload` は JSONB 素通し (kind 別の型付けは別 issue)。UI は alc-app の `AdminDashboard.vue` 「ハブ測定値」タブ (ippoan/alc-app#144)。**session_id (migration 127、Refs ippoan/alc-app-s3#112)**: 1 回の点呼を束ねる端末発番の識別子。端末 (CoreS3) の UI が Measuring → Idle に戻るまでを 1 セッションとして発番し、**端末内でのみ一意** — グローバルな一意性は (tenant_id, device_id, session_id) の組で担保する。**NULL は欠損ではなく「セッション不明」**: 既存行・旧ファーム・点呼外の単発計測 (待機画面で BLE 機器から届いたもの) が該当する。端末由来 = untrusted なので ingest / 一覧の両方で長さ 64 + 英数字 `-` `_` に絞って 400 で弾く (`valid_session_id`)。index は partial (`hub_measurements_tenant_session ... WHERE session_id IS NOT NULL`) — NULL 行を引く用途が無く、伸び続ける ingest テーブルの index を小さく保つため。**`hub_measurements_tenant_kind (tenant_id, kind, created_at DESC)` (migration 135)** は打刻履歴の導出と `GET /api/hub/measurements?kind=` の両方が使う — 既存の `hub_measurements_tenant_device` は先頭の次が `device_id` なので kind 絞りには効かない。**bp_enabled (migration 143、Refs ippoan/alc-app-s3#135)**: 血圧計 (Omron) を使うかどうかの端末設定をサーバ正本にする列。既定は false (既存端末の挙動を変えないため、038 の call_enabled / 050 の always_on と同じ形)。**★ 端末設定の読み出し (`GET /api/devices/settings/{device_id}`) は SECURITY DEFINER 関数 `alc_api.get_device_settings_by_id()` (063/114) 経由でしか `devices` を読めないため、devices に列を足しただけではこの関数の RETURNS TABLE / SELECT 列挙に乗らず応答に出てこない** — 新しい端末設定列を足すときは同じ migration でこの関数側も直すこと |
...
| `alc-vein` | 指静脈の登録と 1:N 照合 (Refs ippoan/vein-match#20)。照合本体 `vein-match-search` (+ core の `vein-match`) を **private repo `ippoan/vein-match` から Cargo の git 依存 (tag `v0.1.1` 固定)** で取り込む。`VeinState { templates: Arc<dyn VeinTemplatesRepository> }` を `src/routes/mod.rs` の `tenant_protected` に `vein::tenant_router().with_state(vein_state)` で merge (router() の引数が 1 つ増えた — main.rs / `tests/common` の `spawn_test_server_with_states` / `tests/mock_helpers` の `setup_mock_vein_state`)。口は 4 本: `PUT /vein/templates/{employee_id}` `{"charas":[hex..]}` (1〜6 件 → `create_template` で 1 件に組んで upsert、キオスクからも) / `POST /vein/identify` `{"chara":hex}` (テナント全件で `Library` を `max(件数,2)` 人で組み `fv_search_user(level=2, t8=unix 秒の下位 8 ビット)`、当たれば `{employee_id,name}`・外れは `{employee_id:null}`、学習後のテンプレートを `UPDATE … WHERE updated_at = 読んだ値` で書き戻し 0 行なら捨てる) / `GET /vein/templates` (`{logic_version, templates:[{employee_id,template,updated_at}]}` — alc-app のオフライン照合用、`logic_version` は wasm の `logic_version()` と突き合わせる) / `DELETE /vein/templates/{employee_id}` (キオスクには auth-worker が開かない)。認可は `update_face` と同じく `TenantId` だけ (役割は auth-worker の許可表)。**特徴量は "BDBD…" の 16 進だけ** (vein-match の `decode_hex` + `load` で検査、外れは 422 `unsupported_chara_format` 等、`alc_core::api_error::unprocessable`)。**501 人以上は分割せず 422 `too_many_templates`** (分割すると DLL と学習・MRU が変わりうる)。**PUT も新規で 500 人を超える登録を同じ 422 で断る** (上書きは人数が増えないので通す。数えは `registration_count` = 一覧と同じく削除済みを除く、判定は `matcher::check_capacity` の 1 か所 — 登録で超えさせるとテナント全員の照合が止まるため)。テンプレートは mode 1 (AES のみ)・pad 空で wasm と同じ。表は `vein_templates` (migration 151、`UNIQUE (tenant_id, employee_id)`、RLS は 034 と同形、GRANT あり)、削除済み乗務員は一覧・照合から除く。ファイル: `matcher.rs` (DB なし。テスト用の合成特徴量 `synth` もここ — vein-match の固定データは private なので public repo に置かない) / `repo.rs` (trait + Pg) / `routes.rs` + `routes_tests.rs` (fake repo で 422・0〜2 人・501 人・書き戻し競合)。実 DB は `tests/vein_templates_test.rs` (bazel `db-vein-templates` shard、coverage `cov-db-vein`)。取得に認証が要る: ローカルは `gh auth setup-git`、CI は cargo/bazel を打つ全 job の checkout 直後に `ippoan/ci-workflows/.github/actions/private-git-auth` (GitHub App token で `url.*.insteadOf`)、reusable の `rust-dep-check` / `catalog-extract` には `private_git_repos: vein-match` + secrets。**cargo/bazel を打つ job を足すときはこの step も足す** (README「private な git 依存」節) |
...
- **router 本体**: `src/routes/mod.rs` — 各 domain crate のルートを re-export し `router()` で結線。
  ...
  dtako-scraper-relay→`/api/dvr/notifications` + `/api/dvr/files/{id}` Refs ohishi-exp/nuxt-dtako-admin#1094)。
...
- **裸の `Err(StatusCode::BAD_REQUEST)` は本文もない 400 になる**: axum の
  `StatusCode` エラー型はレスポンス本文を持たないため、フロントには
  `API エラー (400):` としか出ず原因不明になる (`submit_medical` の 3 箇所で
  2 回本番で誤修正した実害、Refs yhonda-ohishi-alc/alc-app#322/#323、
  ippoan/rust-alc-api#668)。理由を返したいハンドラは戻り値を
  `alc_core::api_error::ApiError` (`(StatusCode, Json<Value>)`) にし、
  `bad_request(error, message)` 等を使う。**ハンドラの戻り型を変えると同じ
  handler 内の他の `StatusCode` エラーも巻き込む**ので、影響範囲をそのハンドラ
  だけに絞ること。
- **血圧必須のボンド判定はヘッダーが正本 (`submit_medical`, Refs #668)**:
  `X-Device-Bp-Bonded: 1`(必須)/`0`(不要)/ヘッダー無し(不明→フェイルクローズ)を
...
| `tests/vein_templates_test.rs` | 指静脈テンプレート (migration 151、Refs ippoan/vein-match#20) — 合成の特徴量 (`alc_vein::matcher::synth`) で登録 → 照合で当たる → 学習後のテンプレートが書き戻され `updated_at` が進む / 同じ乗務員の PUT は 1 行を上書き / 読んだ後に登録し直されたら書き戻しは 0 行で捨てる / 別テナントからは見えず・当たらず・登録できない / 削除済みの乗務員は一覧・照合・登録から除く |
| `tests/dtako_operation_changes_test.rs` | 運行の変更記録 (migration 152、Refs ohishi-exp/nuxt-dtako-admin#1133) — 休憩 0→178 の上げ直しで reupload 1 行 / 同じ中身・初回取り込みは 0 行 / 旧 KUDGIVT が取れない回は分数を比べず、付け替えなどで記録する行の before には `before_kudgivt: "unavailable"` / 手動削除は crew_role ごとに manual_delete・after=NULL / 2マンは crew_role ごとに対応 / 読み口は別 tenant を返さない |
...
| ファイル | エンドポイント |
|---------|-------------|
| `routes/car_inspections.rs` | `GET /api/car-inspections/current`, `/expired`, `/renew`, `/{id}` / `POST /api/car-inspections/lookup` |
...
- **テーブル**: `timecard_cards` (カード:社員 = 多:1)。**`employee_id` は NULL 可** (migration 146、Refs ippoan/rust-alc-api#644) — 社員マスタがまだ同期されていないカードは 持ち主が空で入り、社員番号が `pending_employee_code` に残る (結び付いたら NULL に戻る)。**打刻の行を持つ表は `hub_measurements` だけ** (Refs ippoan/alc-app-s3#134 で一次表をそちらへ寄せた)。旧・打刻表は書き手 (#615 / #618) も読み手 (#616) も無くなったので **#620 で DROP 済み** — 過去の行も残っていない (賃金の監査証跡として残す必要は無い、というオーナー判断)
- **マイグレーション**: `migrations/034_create_timecard.sql` + `migrations/134_timecard_cards_normalize_card_id.sql` (card_id の正規化移行 + CHECK 制約) + `migrations/138_delete_prehistoric_timecard_punches.sql` (1970 年の打刻を削除) + `migrations/139_drop_time_punches.sql` (残置表を DROP) + `migrations/145_timecard_cards_source.sql` (`source` 列の追加 + 初回移行ぶんの backfill。件数を `RAISE NOTICE`、想定超過で `RAISE EXCEPTION` — 138 と同じ形) + `migrations/146_timecard_cards_pending_employee.sql` (`employee_id` の NOT NULL 解除 + `pending_employee_code` 列 + 部分 index `idx_timecard_cards_pending_code`。**`CHECK` 制約は足さない** — 「社員に結び付いていないカード」の拒否は端末 (CoreS3) がその場でやる話で、DB で弾いても呼び出し元には分かりにくい 500 しか返らない)
...
- **バックエンド**: `src/routes/timecard.rs`
  - カード CRUD: `POST/GET /api/timecard/cards`, `DELETE /api/timecard/cards/{id}`
  - **カード台帳の一括取り込み: `PUT /api/timecard/cards/bulk-by-code` (Refs ippoan/rust-alc-api#644)**。既存タイムカード (別システム) の中央 DB にあるカード台帳を、社員番号 (`code`) をキーに `timecard_cards` へ初回移行する受け口。呼び出し元は Cloudflare Worker で、経路は `PUT /employees/bulk-by-code` と同じ (**`tenant_router`**。auth-worker の `forwardAlcTenantData` が `X-Tenant-ID` を注入する — `internal_shared_secret_router` にも `/alc-internal-proxy` の allowlist にも**置かない**)。**単発 CRUD (`POST /timecard/cards`) をループで叩かせない**のが眼目で、`UNIQUE(tenant_id, card_id)` の 409 と code→UUID の解決を呼び出し側 (public repo の Worker) に持たせると判定が二重化する。body は `{dry_run, on_conflict, items[{code, card_id, label}]}` (1〜500 件。上限 `MAX_BULK_UPSERT_ITEMS` は `alc_core::models` で `employees/bulk-by-code` と**共有**。0 件 / 501 件は 400)、応答は `{created, updated, unchanged, pending, skipped[{index, code, reason}]}` で **skip があっても 200** (1 件の不備で数百件の移行を止めない)。処理順は (1) `normalize_card_id` (**唯一の正規化点**。送り手は大文字の生値を送る) → (2) 形の検査 `is_valid_bulk_card_id` = `[0-9a-f]` かつ長さ **8 / 14 / 16** (FeliCa IDm 8 バイトと Mifare UID 4 / 7 バイトが混在するので **16 桁固定にすると一部のカードを黙って落とす**) → (3) バッチ内の重複 card_id は**後勝ちにせず** `duplicate_in_batch` → (4) `SELECT id FROM employees WHERE tenant_id=$1 AND code=$2` (**`deleted_at` は問わない**、`upsert_by_code` と同じ理由) → (5) `INSERT ... ON CONFLICT (tenant_id, card_id) DO UPDATE SET employee_id = EXCLUDED.employee_id, pending_employee_code = EXCLUDED.pending_employee_code WHERE ($6::boolean OR timecard_cards.employee_id IS NULL) AND (... IS DISTINCT FROM ...) RETURNING (xmax = 0)` の 1 文で新規 (`created`) / 付け替え (`updated`) / 同じ持ち主 (`unchanged`) を分ける。skip 理由は 3 種 `invalid_card_id` / `card_owner_conflict` (既定。`on_conflict:"reassign"` のときだけ付け替える) / `duplicate_in_batch`。**★ 社員が引けなくても skip しない (Refs ippoan/rust-alc-api#644)**: 社員マスタは別経路 (デジタコ relay の `PUT /employees/bulk-by-code`) が別スケジュールで入れるので「カードが先・社員が後」が普通に起きる。落とすと拾い直す仕組みが無く**そのカードは永久に alc に入らない**ので、`employee_id = NULL` + `pending_employee_code = <code>` で受け入れ、件数を `pending` で返す (**行は作られているので `created` にも数える。`skipped` には入れない**)。結び付けは社員が現れる側 = `alc-misc/src/repo/employees.rs` の `link_pending_cards` が `upsert_by_code` と**同じトランザクション内**で行い、`upsert_by_code` の **4 つの成功経路すべて** (nfc_id 衝突 / 通常 update / nfc_id 一致で code を付ける / 新規 INSERT) から呼ぶ。**比較は `IS DISTINCT FROM`** (`<>` は片側 NULL で NULL = false 扱いになり付け替えが黙って外れる)、**持ち主の居ない行は `on_conflict` を待たずに引き取る** (守るべき別の持ち主が居ないため。社員が relay 以外の経路で現れたときにここが待つと「受け入れたのに紐づけできない」が残る)、**`DO UPDATE` の SET に `pending_employee_code` を必ず含める** (落とすと付け替え後の行に古い code が残り、`employee_id IS NULL` しか見ない結び付けが拾えず部分 index に幽霊行が残る)。**`unchanged` の持ち主照合は `Option<(Option<Uuid>,)>` で受ける** — 保留行の NULL を `Uuid` にデコードできず **1 件で tx ごと 500** になる。**`skipped` に `card_id` を載せない・`tracing` にも出さない** — 呼び出し元が public repo の Worker で、応答がそのままログや issue に写るため (`index` と `code` で行は特定できる)。`dry_run:true` は**判定を本番と同じコードで通してから tx を commit しない**形 (判定コードを 2 本持たない)。**1 件のせいで tx ごと落とさない**: 解決は SELECT、書き込みは ON CONFLICT、形の不正は事前 validate なので制約違反の余地が無く savepoint も要らない。pure fn (`normalize` → `is_valid_bulk_card_id` → `prepare_bulk_cards`) は `alc-core/src/repository/timecard.rs`、SQL は `alc-misc/src/repo/timecard.rs` の `bulk_upsert_cards_by_code`、handler は `alc-misc/src/timecard.rs`
  - **カード台帳の継続同期 (削除側): `POST /api/timecard/cards/delete-by-card` (Refs ippoan/rust-alc-api#644)**。初回移行のあと、外部の画面でカードを登録・削除したときに 1 枚ずつ反映するための受け側。**追加は `PUT /timecard/cards/bulk-by-code` を items 1 件で再利用する**ので、専用に足したのはこの削除だけ。置き場所は同じ **`tenant_router`**。body は `{card_id, dry_run}` (card_id は読み取った生値でよい)、応答は `{deleted: 0|1, reason, code}` で **reason は `deleted` / `not_found` / `out_of_scope` / `invalid_card_id` の 4 種**。**範囲外・不在・形不正はどれも 200** — 404 にすると呼び出し元の画面が「消えた」と誤解する文面を出す。`code` は `employees.code` = **誰のカードを外したか**を画面に出すためだけに返す (保留行は `employees` への JOIN が空振りするので **`LEFT JOIN` + `COALESCE(e.code, gone.pending_employee_code)`** で埋める — `INNER JOIN` のままだと未結び付きカードを外したとき画面が無名になる)。**`card_id` は応答にも `tracing` にも出さない** (bulk と同じ理由)。正規化は `normalize_card_id`、形の検査は `is_valid_bulk_card_id` を**再利用**する (2 実装目を作ると「同期では入るのに削除では当たらない」が生まれる)。SQL は `DELETE ... WHERE tenant_id=$1 AND card_id=$2 AND source=$3 RETURNING employee_id, pending_employee_code` を CTE に入れた **1 文**で、同じスナップショットの `SELECT` と並べて「消えた / 出所が違う / そもそも無い」を分ける (SELECT してから DELETE すると間に付け替えが入る)。`dry_run` は bulk と同じ「判定は本番と同じコードを通し commit しない」形。**★ 登録は POST 1 つだけ**にして他 method を axum に 405 で閉じさせている — 呼び出し元 (auth-worker の転送 allowlist) は **method を見ず path だけで通す**ので、閉じるのはこちら側の責務
  - **★ 削除の射程を決めるのは `timecard_cards.source` (migration 145、Refs ippoan/rust-alc-api#644)**。`timecard_cards` には**この同期と無関係に alc 側で直接登録されたカードが在り得る**ので、card_id だけで無条件に消すとそれを巻き込む。値は**サーバ側の定数** `alc_core::repository::timecard::CARD_SOURCE_LEDGER_SYNC` (= 初回移行で使った label と同じ文字列) で、**body からは絶対に取らない**。刻むのは `bulk_upsert_cards_by_code` の INSERT だけ — 単発の `POST /timecard/cards` は刻まない (= NULL のまま = **射程外**)。**既存の `label` を根拠にしてはいけない**: 自由文で誰でも書ける / `ON CONFLICT ... DO UPDATE` が `label` を更新しないので偽陰性が既に在る / 人が UI から同じ文字列を打てば偽陽性になる、の 3 つ。**★ `ON CONFLICT ... DO UPDATE` の SET に `source` を足さないこと**: alc 側で先に登録された行 (`source IS NULL`) は同期が同じ社員のまま再確認しても NULL のままだが (unchanged 経路は 1 行も書かないため)、これは **fail-closed 側 = 消せないだけの仕様**で、直そうと `SET source` を足すと unchanged が UPDATE になり**実測済みの冪等性 (2 回目が created:0 / unchanged:120) が壊れる**。消したい行が出たら同期側で 1 度外して入れ直す (新規 INSERT になり刻まれる)。`tests/timecard_cards_bulk_test.rs` の `test_unchanged_row_keeps_null_source_by_design` が仕様として固定している。カードを消しても `hub_measurements` の凍結済み `payload.employee_id` は消えない (打刻履歴は道連れにしない)
  - 打刻: `POST /api/timecard/punch` (card_id → 社員特定 → 打刻 + 当日一覧返却)。**書き込み先は `hub_measurements`** ただ 1 つ (Refs ippoan/alc-app-s3#134)。端末が NVS の連番を持つのに対しブラウザは持たないので、`seq` は **sequence `hub_measurements_browser_seq` の `nextval`** で採番する (migration 137)。`MAX(seq)+1` は同時打刻で unique 違反のリトライループが要る。**この経路に冪等性は無い** (同じ seq の再送という概念が無いため) ので、二度押しは 2 行になる — 旧・打刻表への INSERT も同じで退行ではない。`employee_id` は端末側と同じく **payload に凍結**する。`device_id` は hub 側が TEXT NOT NULL なので、キオスクの UUID が無ければ `'browser'` に倒す
  - 一覧/CSV: `GET /api/timecard/punches`, `GET /api/timecard/punches/csv`
- **★ 打刻の一次表は `hub_measurements` で、一覧は導出する (Refs ippoan/alc-app-s3#134)**。**打刻の表は 1 つだけ** — かつて置いていたコピーのせいで「時刻がサーバ時刻になる」「端末 ID が入らない」「重複排除が要る」が生まれた。書き手も読み手も無くなったので #620 で DROP 済み (index / FK / RLS ポリシーは表に従属するので `DROP TABLE` 一発で消える。migration 139 は DROP の前に両表の件数を `RAISE NOTICE` で出し、書き手撤去後に増えた行があれば `RAISE EXCEPTION` で中止する)。SQL は `alc-misc/src/repo/timecard.rs` の `PUNCHES_CTE` 1 本を `count_punches` / `list_punches` / `list_punches_for_csv` が共有する。**社員の解決は 1 本の COALESCE**: `payload.employee_id` (ingest で凍結済み) → `timecard_cards` → `employees.nfc_id`。**`timecard` だけ凍結**するのは `timecard_cards` が hard DELETE + UNIQUE で付け替えが「削除→再登録」になり、読むたび解決すると退職者の過去の打刻が新人に付くため。`license` は免許証番号が人に固定で付け替え問題が無いので凍結せず毎回引く (経路は分岐させない)。おまけで**未解決の timecard 行は後からカード登録すると自動で拾われる** (backfill 不要)。**`payload.employee_id` は正規表現で UUID を確かめてから cast する** — 素で `::uuid` すると 1 行のせいで一覧全体が 500 になる。**JOIN 内の card_id 正規化は `normalize_card_id` の SQL 版**なので、片方だけ変えると照合が静かに外れる。`punched_at` は `COALESCE(recorded_at, created_at)` (端末計時が第一、時計未同期の NULL は到着時刻に倒す) で、**絞り込みと並びもこれ** — `created_at` で絞ると回線断で溜めた朝の打刻が夕方の行として並ぶ。index は `hub_measurements_tenant_kind_punched_at` (migration 136、式インデックス)。`employee_id` / `employee_name` は **Option** (未登録カードのタップも行として出す — 落とすと登録漏れに気付けない)。`device_id` は常に NULL で、端末は `device_name` に hub の device_id 文字列が入る。**`kind` (`timecard` / `license`) は必須の区分列** — 一覧は打刻と点呼を両方返すので (始業点呼 = 始業打刻)、これが無いと画面も CSV も点呼を打刻として扱う。CSV は「区分」列 (打刻 / 点呼) として出す。**`card_kind` は `kind` とは別の軸で「免許証か、他の IC カードか」 (Refs ippoan/rust-alc-api#644)**: 端末のファームは打刻の `kind` を**常に `timecard` 固定**で送るので、カードの種別は `payload.card_kind` (`felica_idm` / `nfca_uid` / `license`) にしか入っていない。CTE は `COALESCE(hm.payload->>'card_kind', CASE WHEN hm.kind = 'license' THEN 'license' END) AS card_kind` で取り出し、`kind='license'` (点呼) の行も免許証として出す。**`kind` を流用しないこと。**ブラウザ打刻 (`POST /timecard/punch`) と旧行は `card_kind` が **null のままで正しい** (ブラウザは何のカードで打ったかを知らない)。**表示専用で、社員の解決 (照合) には絶対に使わない** — 使った瞬間ブラウザ版と端末経由で挙動が割れる。CSV は「カード」列 (`csv_card_kind_label`: `license`→免許証 / `felica_idm`・`nfca_uid`→ICカード / **未知はそのまま** / None→空欄) として出し、**「区分」列は消さない** (両方無いと「点呼の免許証」と「打刻の免許証」が区別できない)。**`list_punches` と `list_punches_for_csv` の SELECT は両方に足すこと** — 片方だけだと画面か CSV のどちらかが静かに欠ける。**`PUNCHES_CTE` に bind を増やさないこと** — `build_punch_where` が `$2` 以降を自前で採番するので、`list_punches` / `count_punches` / `list_punches_for_csv` / `list_today_punches` の 4 か所全部で bind 順がずれる
- **テスト**: 読み出し SQL は `tests/timecard_punches_test.rs`、一括取り込みの `ON CONFLICT` と card_id の CHECK 制約、`source` を刻む経路と削除の射程は `tests/timecard_cards_bulk_test.rs` (どちらも**実 DB**。削除の口のテストも**新規ファイルを作らずここに追記する** — Bazel matrix 登録が 3 か所要るため) が固定する。`tests/mock_tests/mock_timecard_test.rs` は repository を差し替えるので **SQL を 1 行も通らない** — CTE を触ったらこちらだけ通っても意味がない
- **★ `coverage_100.toml` 登録ファイルに `match` を足したら分岐テストが要る。** 式に埋めるだけ (`foo(&normalize(x))` のような呼び出しの追加) なら分岐は増えないので不要 — **この区別を取り違えると gate だけが落ちる**。2026-09-04 に `csv_kind_label` の `match` で実際に踏んだ (直前の PR が「式に埋めただけ」だったのを、そのまま次にも当てはめた)
- **★ 実 DB テストは `BUILD.bazel` に `rust_test` を登録しないと CI で 1 度も走らない。** CI のテストは Bazel のターゲット単位なので、`tests/` にファイルを置いて `cargo test` が通っても**CI では 0 件**。`scripts/check_bazel_test_matrix.sh` は「ターゲットと matrix の対応」を見る検査なので、**ターゲット自体が無い状態は検知できない** (28/28 で整合してしまう)。登録は 3 か所 — `BUILD.bazel` の `rust_test` / `.github/workflows/ci.yml` の `bazel-test-db` matrix / `cache-warm.yml` の同 matrix (後の 2 つは pair 一致が検査される)。**2026-09-04 に `timecard_punches_test` で実際に踏んだ** (「テストが実行されないまま本番に出る」を防ぐために足したテストが、それ自身で起きていた)
- **フロントエンド**:
...
**対象外:** measurements, hub_measurements, tenko_sessions, tenko_records, file_access_logs（履歴データ）
