...
- **テスト**: 読み出し SQL は `tests/timecard_punches_test.rs` (**実 DB**) が固定する。`tests/mock_tests/mock_timecard_test.rs` は repository を差し替えるので **SQL を 1 行も通らない** — CTE を触ったらこちらだけ通っても意味がない
