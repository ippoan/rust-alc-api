...
- **マイグレーション**: `migrations/034_create_timecard.sql` + `migrations/134_timecard_cards_normalize_card_id.sql` (card_id の正規化移行 + CHECK 制約) + `migrations/138_delete_prehistoric_timecard_punches.sql` (1970 年の打刻を削除) + `migrations/139_drop_time_punches.sql` (残置表を DROP)
...
- **テスト**: 読み出し SQL は `tests/timecard_punches_test.rs`、一括取り込みの `ON CONFLICT` と card_id の CHECK 制約は `tests/timecard_cards_bulk_test.rs` (どちらも**実 DB**) が固定する。`tests/mock_tests/mock_timecard_test.rs` は repository を差し替えるので **SQL を 1 行も通らない** — CTE を触ったらこちらだけ通っても意味がない
