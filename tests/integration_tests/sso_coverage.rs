...
async fn setup_sso_admin() -> (
    ...
    .await;
    let jwt =
        crate::common::create_test_jwt_for_user(user_id, tenant_id, "ssoadmin@test.com", "admin");
...
async fn test_sso_upsert_encrypt_error() {
    ...
    test_case!("JWT_SECRET未設定 → 暗号化エラー → 500", {
        ...
        .await;
        let jwt =
            crate::common::create_test_jwt_for_user(user_id, tenant_id, "ssoenc@test.com", "admin");
        let client = reqwest::Client::new();
