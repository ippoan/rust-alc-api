...
async fn test_get_scrape_history_empty() {
    ...
    test_case!("履歴が空 → 空配列を返す", {
        ...
        let tenant_id = crate::common::create_test_tenant(state.pool(), "Scraper Hist").await;
...
async fn test_get_scrape_history_with_data() {
    ...
    test_case!("履歴あり → limit/offset 対応", {
        ...
        let state = crate::common::setup_app_state().await;
        let base_url = crate::common::spawn_test_server(state.clone()).await;
        let tenant_id = crate::common::create_test_tenant(state.pool(), "Scraper Hist2").await;
        let jwt = crate::common::create_test_jwt(tenant_id, "admin");
...
async fn test_trigger_scrape_connection_error() {
    ...
    test_case!("scraper 接続不可 → 502", {
        ...
        let state = crate::common::setup_app_state().await;
        let base_url =
            crate::common::spawn_test_server_with_scraper(state.clone(), "http://127.0.0.1:19999")
                .await;
        let tenant_id = crate::common::create_test_tenant(state.pool(), "Scraper Err").await;
        let jwt = crate::common::create_test_jwt(tenant_id, "admin");
...
async fn test_trigger_scrape_scraper_error_response() {
    ...
    test_case!("scraper 500 レスポンス → 502", {
        ...
        let state = crate::common::setup_app_state().await;
        let base_url =
            crate::common::spawn_test_server_with_scraper(state.clone(), &mock_server.uri()).await;
        let tenant_id = crate::common::create_test_tenant(state.pool(), "Scraper 500").await;
        let jwt = crate::common::create_test_jwt(tenant_id, "admin");
...
async fn test_trigger_scrape_sse_happy_path() {
    ...
    test_case!("scraper SSE レスポンスをプロキシ + DB 保存", {
        ...
        let base_url =
            crate::common::spawn_test_server_with_scraper(state.clone(), &mock_server.uri()).await;
        let tenant_id = crate::common::create_test_tenant(state.pool(), "Scraper SSE").await;
        let jwt = crate::common::create_test_jwt(tenant_id, "admin");
...
async fn test_trigger_scrape_default_date() {
    ...
    test_case!(
        "start_date なし → デフォルト昨日 + skip_upload",
        {
            ...
            let state = crate::common::setup_app_state().await;
            let base_url =
                crate::common::spawn_test_server_with_scraper(state.clone(), &mock_server.uri())
                    .await;
            let tenant_id = crate::common::create_test_tenant(state.pool(), "Scraper Def").await;
            let jwt = crate::common::create_test_jwt(tenant_id, "admin");
        ...
        "不正な start_date → パース失敗 → 今日にフォールバック",
        {
            ...
            let base_url =
                crate::common::spawn_test_server_with_scraper(state.clone(), &mock_server.uri())
                    .await;
            let tenant_id =
                crate::common::create_test_tenant(state.pool(), "Scraper BadDate").await;
            let jwt = crate::common::create_test_jwt(tenant_id, "admin");
...
async fn test_trigger_scrape_with_id_token() {
    ...
    test_case!(
        "メタデータサーバーから ID トークン取得 → Bearer 付与",
        {
            ...
            let state = crate::common::setup_app_state().await;
            let base_url =
                crate::common::spawn_test_server_with_scraper(state.clone(), &scraper_server.uri())
                    .await;
            let tenant_id = crate::common::create_test_tenant(state.pool(), "Scraper Token").await;
            let jwt = crate::common::create_test_jwt(tenant_id, "admin");
        ...
        "メタデータサーバー 403 → トークンなしで続行",
        {
            ...
            let base_url =
                crate::common::spawn_test_server_with_scraper(state.clone(), &scraper_server.uri())
                    .await;
            let tenant_id =
                crate::common::create_test_tenant(state.pool(), "Scraper MetaErr").await;
            let jwt = crate::common::create_test_jwt(tenant_id, "admin");
    ...
    test_case!(
        "SSE: 空データ行 + non-data 行 + comp_id なし result",
        {
            ...
            let base_url =
                crate::common::spawn_test_server_with_scraper(state.clone(), &mock_server.uri())
                    .await;
            let tenant_id = crate::common::create_test_tenant(state.pool(), "Scraper Edge").await;
            let jwt = crate::common::create_test_jwt(tenant_id, "admin");
...
async fn test_get_scrape_history_db_error() {
    ...
    test_case!(
        "dtako_scrape_history テーブル RENAME → DB エラー → 500",
        {
            ...
            let _flock = crate::common::db_rename_flock();
            let state = crate::common::setup_app_state().await;
            let tenant_id = crate::common::create_test_tenant(state.pool(), "Scraper DB Err").await;
            let jwt = crate::common::create_test_jwt(tenant_id, "admin");
            let base_url = crate::common::spawn_test_server(state.clone()).await;
...
async fn test_trigger_scrape_client_disconnect() {
    ...
    test_case!("SSE ストリーム中にクライアント切断", {
        ...
        let base_url =
            crate::common::spawn_test_server_with_scraper(state.clone(), &mock_server.uri()).await;
        let tenant_id = crate::common::create_test_tenant(state.pool(), "Scraper Disc").await;
        let jwt = crate::common::create_test_jwt(tenant_id, "admin");
...
async fn test_trigger_scrape_stream_error() {
    ...
    test_case!("不正な chunked encoding → bytes_stream Err", {
        ...
        let state = crate::common::setup_app_state().await;
        let base_url =
            crate::common::spawn_test_server_with_scraper(state.clone(), &format!("http://{addr}"))
                .await;
        let tenant_id = crate::common::create_test_tenant(state.pool(), "Scraper StreamErr").await;
        let jwt = crate::common::create_test_jwt(tenant_id, "admin");
