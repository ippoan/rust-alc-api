...
async fn setup() -> (
    ...
    let tenant_id = crate::common::create_test_tenant(state.pool(), "Meas Test").await;
...
async fn setup_with_employee() -> (String, String, String, reqwest::Client) {
    ...
    let emp =
        crate::common::create_test_employee(&client, &base_url, &auth, "TestEmp", "TE01").await;
...
async fn test_list_measurements() {
    ...
    test_case!("測定一覧取得", {
        ...
        crate::common::create_test_measurement(&client, &base_url, &auth, &emp_id).await;
        crate::common::create_test_measurement(&client, &base_url, &auth, &emp_id).await;
...
async fn test_list_filter_by_employee() {
    ...
    test_case!("従業員でフィルタ", {
        ...
        let emp1 =
            crate::common::create_test_employee(&client, &base_url, &auth, "Emp1", "E01").await;
        let emp2 =
            crate::common::create_test_employee(&client, &base_url, &auth, "Emp2", "E02").await;
        let id1 = emp1["id"].as_str().unwrap();
        let id2 = emp2["id"].as_str().unwrap();

        crate::common::create_test_measurement(&client, &base_url, &auth, id1).await;
        crate::common::create_test_measurement(&client, &base_url, &auth, id2).await;
        crate::common::create_test_measurement(&client, &base_url, &auth, id1).await;
...
async fn test_measurement_tenant_isolation() {
    ...
    test_case!("テナント分離", {
        ...
        let tenant_a = crate::common::create_test_tenant(state.pool(), "Meas Iso A").await;
        let tenant_b = crate::common::create_test_tenant(state.pool(), "Meas Iso B").await;
        ...
        // テナント A に従業員+測定作成
        let emp =
            crate::common::create_test_employee(&client, &base_url, &auth_a, "EmpA", "EA1").await;
        let emp_id = emp["id"].as_str().unwrap();
        let m = crate::common::create_test_measurement(&client, &base_url, &auth_a, emp_id).await;
