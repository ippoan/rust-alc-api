...
async fn setup_admin() -> (
    ...
    .await;
    let (user_id, _) =
        crate::common::create_test_user_in_db(state.pool(), tenant_id, "admin@test.com", "admin")
            .await;
    let jwt =
        crate::common::create_test_jwt_for_user(user_id, tenant_id, "admin@test.com", "admin");
...
async fn test_delete_user() {
    ...
    test_case!("ユーザーを削除", {
        ...
        // 削除用ユーザーを作成
        let (target_id, _) = crate::common::create_test_user_in_db(
            state.pool(),
            tenant_id,
            "target@test.com",
            "viewer",
...
async fn test_users_forbidden_for_viewer() {
    ...
    test_case!("viewerロールでユーザー一覧が403", {
        let state = crate::common::setup_app_state().await;
        let base_url = crate::common::spawn_test_server(state.clone()).await;
        let tenant_id = crate::common::create_test_tenant(state.pool(), "ViewerForbid").await;
...
async fn test_bot_forbidden_for_viewer() {
    ...
    test_case!("viewerロールでBot管理が403", {
        ...
        let (viewer_id, _) = crate::common::create_test_user_in_db(
            state.pool(),
            tenant_id,
            "botviewer@test.com",
            "viewer",
        ...
        .await;
        let viewer_jwt = crate::common::create_test_jwt_for_user(
            viewer_id,
            tenant_id,
            "botviewer@test.com",
            "viewer",
...
async fn test_delete_user_self_delete() {
    ...
    test_case!("自分自身の削除は400", {
        ...
        let (self_id, _) = crate::common::create_test_user_in_db(
            state.pool(),
            tenant_id,
            "selfdelete@test.com",
            "admin",
        ...
        .await;
        let self_jwt = crate::common::create_test_jwt_for_user(
            self_id,
            tenant_id,
            "selfdelete@test.com",
            "admin",
...
async fn test_bot_upsert_encrypt_error_no_key() {
    ...
    test_case!("暗号化キーなしでupsertが500", {
        ...
        let state = crate::common::setup_app_state().await;
        let base_url = crate::common::spawn_test_server(state.clone()).await;
        let tenant_id = crate::common::create_test_tenant(state.pool(), "BotNoKey").await;
        let (user_id, _) = crate::common::create_test_user_in_db(
            state.pool(),
            tenant_id,
            "botnokey@test.com",
