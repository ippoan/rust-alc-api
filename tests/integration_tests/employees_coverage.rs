...
async fn test_create_employee_db_error() {
    ...
    test_case!("create_employee: INSERT trigger → 500", {
        ...
        let tenant_id = crate::common::create_test_tenant(state.pool(), "EmpCreateErr").await;
...
async fn test_delete_employee_db_error() {
    ...
    test_case!("delete_employee: UPDATE trigger → 500", {
        ...
        let state = crate::common::setup_app_state().await;
        let base_url = crate::common::spawn_test_server(state.clone()).await;
        let tenant_id = crate::common::create_test_tenant(state.pool(), "EmpDelErr").await;
        let jwt = crate::common::create_test_jwt(tenant_id, "admin");
        let auth = format!("Bearer {jwt}");
        let client = reqwest::Client::new();
        ...
        let emp =
            crate::common::create_test_employee(&client, &base_url, &auth, "DelErrEmp", "DE01")
                .await;
...
async fn test_update_face_db_error() {
    ...
    test_case!("update_face: UPDATE trigger → 500", {
        ...
        let state = crate::common::setup_app_state().await;
        let base_url = crate::common::spawn_test_server(state.clone()).await;
        let tenant_id = crate::common::create_test_tenant(state.pool(), "FaceErr").await;
        let jwt = crate::common::create_test_jwt(tenant_id, "admin");
        let auth = format!("Bearer {jwt}");
        let client = reqwest::Client::new();

        let emp =
            crate::common::create_test_employee(&client, &base_url, &auth, "FaceErrEmp", "FE01")
                .await;
        let id = emp["id"].as_str().unwrap();
...
async fn test_update_license_db_error() {
    ...
    test_case!("update_license: UPDATE trigger → 500", {
        ...
        let state = crate::common::setup_app_state().await;
        let base_url = crate::common::spawn_test_server(state.clone()).await;
        let tenant_id = crate::common::create_test_tenant(state.pool(), "LicErr").await;
        let jwt = crate::common::create_test_jwt(tenant_id, "admin");
        let auth = format!("Bearer {jwt}");
        let client = reqwest::Client::new();

        let emp =
            crate::common::create_test_employee(&client, &base_url, &auth, "LicErrEmp", "LE01")
                .await;
        let id = emp["id"].as_str().unwrap();
...
async fn test_update_nfc_id_db_error() {
    ...
    test_case!("update_nfc_id: UPDATE trigger → 500", {
        ...
        let emp =
            crate::common::create_test_employee(&client, &base_url, &auth, "NfcErrEmp", "NE01")
                .await;
        let id = emp["id"].as_str().unwrap();
...
async fn test_approve_face_db_error() {
    ...
    test_case!("approve_face: UPDATE trigger → 500", {
        ...
        let state = crate::common::setup_app_state().await;
        let base_url = crate::common::spawn_test_server(state.clone()).await;
        let tenant_id = crate::common::create_test_tenant(state.pool(), "AppFaceErr").await;
        let jwt = crate::common::create_test_jwt(tenant_id, "admin");
        let auth = format!("Bearer {jwt}");
        let client = reqwest::Client::new();

        let emp =
            crate::common::create_test_employee(&client, &base_url, &auth, "AppFaceEmp", "AF01")
                .await;
        let id = emp["id"].as_str().unwrap();
...
async fn test_reject_face_db_error() {
    ...
    test_case!("reject_face: UPDATE trigger → 500", {
        ...
        let state = crate::common::setup_app_state().await;
        let base_url = crate::common::spawn_test_server(state.clone()).await;
        let tenant_id = crate::common::create_test_tenant(state.pool(), "RejFaceErr").await;
        let jwt = crate::common::create_test_jwt(tenant_id, "admin");
        let auth = format!("Bearer {jwt}");
        let client = reqwest::Client::new();

        let emp =
            crate::common::create_test_employee(&client, &base_url, &auth, "RejFaceEmp", "RF01")
                .await;
        let id = emp["id"].as_str().unwrap();
