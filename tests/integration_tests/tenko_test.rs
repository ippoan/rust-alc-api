...
async fn test_equipment_failure_create_db_error() {
    ...
    test_case!("create_failure: trigger → 500", {
        ...
        let state = crate::common::setup_app_state().await;
        let base_url = crate::common::spawn_test_server(state.clone()).await;
        let tenant_id = crate::common::create_test_tenant(state.pool(), "EqFailErr").await;
        let jwt = crate::common::create_test_jwt(tenant_id, "admin");
        let auth = format!("Bearer {jwt}");
        let client = reqwest::Client::new();
...
async fn test_equipment_failure_list_db_error() {
    ...
    test_case!("list_failures: RENAME → 500", {
        ...
        let state = crate::common::setup_app_state().await;
        let base_url = crate::common::spawn_test_server(state.clone()).await;
        let tenant_id = crate::common::create_test_tenant(state.pool(), "EqListErr").await;
        let jwt = crate::common::create_test_jwt(tenant_id, "admin");
        let auth = format!("Bearer {jwt}");
        let client = reqwest::Client::new();
...
async fn test_tenko_schedule_create_db_error() {
    ...
    test_case!("create_schedule: trigger → 500", {
        ...
        let state = crate::common::setup_app_state().await;
        let base_url = crate::common::spawn_test_server(state.clone()).await;
        let tenant_id = crate::common::create_test_tenant(state.pool(), "SchedCreateErr").await;
        let jwt = crate::common::create_test_jwt(tenant_id, "admin");
        let auth = format!("Bearer {jwt}");
        let client = reqwest::Client::new();
        ...
        let emp =
            crate::common::create_test_employee(&client, &base_url, &auth, "SchedErrEmp", "SE001")
                .await;
        let emp_id = emp["id"].as_str().unwrap();
...
async fn test_tenko_schedule_list_db_error() {
    ...
    test_case!("list_schedules: RENAME → 500", {
        ...
        let state = crate::common::setup_app_state().await;
        let base_url = crate::common::spawn_test_server(state.clone()).await;
        let tenant_id = crate::common::create_test_tenant(state.pool(), "SchedListErr").await;
        let jwt = crate::common::create_test_jwt(tenant_id, "admin");
        let auth = format!("Bearer {jwt}");
        let client = reqwest::Client::new();
...
async fn test_tenko_schedule_update_db_error() {
    ...
    test_case!("update_schedule: trigger → 500", {
        ...
        let state = crate::common::setup_app_state().await;
        let base_url = crate::common::spawn_test_server(state.clone()).await;
        let tenant_id = crate::common::create_test_tenant(state.pool(), "SchedUpdErr").await;
        let jwt = crate::common::create_test_jwt(tenant_id, "admin");
        let auth = format!("Bearer {jwt}");
        let client = reqwest::Client::new();
        ...
        let emp =
            crate::common::create_test_employee(&client, &base_url, &auth, "SchedUpdEmp", "SU001")
                .await;
        let emp_id = emp["id"].as_str().unwrap();
        let res = client
            .post(format!("{base_url}/api/tenko/schedules"))
...
async fn test_tenko_schedule_delete_db_error() {
    ...
    test_case!("delete_schedule: trigger → 500", {
        ...
        let state = crate::common::setup_app_state().await;
        let base_url = crate::common::spawn_test_server(state.clone()).await;
        let tenant_id = crate::common::create_test_tenant(state.pool(), "SchedDelErr").await;
        let jwt = crate::common::create_test_jwt(tenant_id, "admin");
        let auth = format!("Bearer {jwt}");
        let client = reqwest::Client::new();
        ...
        // Create employee + schedule first
        let emp =
            crate::common::create_test_employee(&client, &base_url, &auth, "SchedDelEmp", "SD001")
                .await;
        let emp_id = emp["id"].as_str().unwrap();
        let res = client
            .post(format!("{base_url}/api/tenko/schedules"))
...
async fn test_start_session_employee_mismatch() {
    ...
    test_case!(
        "スケジュールの従業員と異なる従業員で開始 → 400",
        {
            ...
            let emp1 = crate::common::create_test_employee(
                &client,
                &base_url,
                &auth,
                "EmpA",
            ...
            let emp2 = crate::common::create_test_employee(
                &client,
                &base_url,
                &auth,
                "EmpB",
