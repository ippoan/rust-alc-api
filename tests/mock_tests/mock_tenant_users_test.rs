...
async fn setup() -> (String, String, uuid::Uuid, uuid::Uuid) {
    ...
    let jwt =
        crate::common::create_test_jwt_for_user(user_id, tenant_id, "admin@example.com", "admin");
...
async fn test_list_users_with_data() {
    ...
    let mock = std::sync::Arc::new(crate::mock_helpers::MockTenantUsersRepository::default());
