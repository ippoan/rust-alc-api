//! FCM v1 API クライアント (adapter)。
//!
//! port (`FcmSenderTrait` / `FcmError`) は `alc_core::fcm` に居る。認証は
//! Cloud Run metadata server の OAuth2 access token (ADC 相当) を使う。

use alc_core::fcm::{FcmError, FcmSenderTrait};
use reqwest::Client;
use serde::Serialize;
