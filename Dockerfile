...
# Copy workspace manifests for dependency caching (layer cache — works on CI)
...
# Dummy sources → build external deps only (cached as Docker layer)
...
# Copy real sources and build (only workspace crates recompile)
COPY src ./src
COPY crates ./crates
